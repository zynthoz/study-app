...
export const Exams: React.FC = () => {
  ...
  // Tabs & Sharing States
  const [activeExamsTab, setActiveExamsTab] = useState<'my' | 'shared'>('my')
  ...
  const openShareModal = async (examId: string) => {
  ...
  const handleAddShare = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!sharingExamId || !shareEmail.trim()) return
    try {
      const { data, error: shareError } = await supabase
        .from('tbl_shares')
        .insert({
          exam_id: sharingExamId,
          sender_id: user?.id,
  ...
  const filteredExams = exams.filter((e) => {
    ...
    if (activeExamsTab === 'my' && !isOwner) return false
    if (activeExamsTab === 'shared' && isOwner) return false
    ...
    <div className="mx-auto max-w-7xl px-6 py-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6 pb-4 border-b border-white/5">
        <div className="flex items-center gap-4">
          ...
          <button
            onClick={() => setActiveExamsTab('shared')}
            className={`text-sm font-bold pb-2 border-b-2 transition-all cursor-pointer ${
              activeExamsTab === 'shared'
                ? 'text-white border-purple-500'
                : 'text-zinc-400 border-transparent hover:text-zinc-200'
            }`}
          >
                ...
                <div className="double-bezel-inner p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  ...
                  <div className="flex items-center justify-between sm:justify-end gap-3 shrink-0">
                    ...
                    <div className="flex items-center gap-1">
                      {/* Share Button (only for own exams) */}
                      {exam.user_id === user?.id && (
                        <button
                          onClick={(e) => {
                            e.preventDefault()
                            e.stopPropagation()
                            openShareModal(exam.id)
                          }}
                          className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-white/0 border border-transparent text-zinc-500 hover:bg-white/10 hover:text-purple-400 transition-all cursor-pointer opacity-0 group-hover:opacity-100 focus:opacity-100"
                          title="Share Exam"
                        >
                      ...
                      {/* Delete Button (only for own exams) */}
                      {exam.user_id === user?.id && (
                        <button
